    ...
    switch (src->type) {
        ...
        case LEPT_ARRAY:
            {
                ...
                dst->u.a.size = 0;
                for (; ix < src->u.a.size; ++ix)
                {
                    lept_copy(&dst->u.a.e[ix], &src->u.a.e[ix]);
                    ++dst->u.a.size;
                }
                dst->type = LEPT_ARRAY;
            }
        ...
        case LEPT_OBJECT:
            /* \todo */
            {
                ...
                dst->u.o.size = 0;
                for (; ix < src->u.o.size; ++ix)
                {
                    dst->u.o.m[ix].k = (char*)malloc(sizeof(char) * src->u.o.m[ix].klen);
                    memcpy(dst->u.o.m[ix].k, src->u.o.m[ix].k, src->u.o.m[ix].klen);
                    dst->u.o.m[ix].klen = src->u.o.m[ix].klen;
                    lept_copy(&dst->u.o.m[ix].v, &src->u.o.m[ix].v);
                    ++dst->u.o.size;
                }
    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; ++i){
                lept_value * rhsv_found;
                if(NULL == (rhsv_found = lept_find_object_value(rhs,lhs->u.o.m[i].k,lhs->u.o.m[i].klen)))return 0;
                else if(lept_is_equal(&lhs->u.o.m[i].v , rhsv_found) != 1)return 0;
            }
            return 1;
        default:
            return 1;
...
lept_value* lept_insert_array_element(lept_value* v, size_t index) {
    ...
    /* \todo */
    if( v->u.a.capacity <= v->u.a.size){
        lept_reserve_array(v, v->u.a.size + v->u.a.size>>1);
    }
    size_t count = v->u.a.size - index;
    for(;count > 0;--count)
    {
        lept_move(&v->u.a.e[index+count],&v->u.a.e[index+count-1]);
    }
    ++v->u.a.size;
    return &v->u.a.e[index];
}
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    ...
    size_t ix = index;
    if(count <= 0)return;
    for(;ix+count < v->u.a.size ;++ix)
    {
        lept_swap(&v->u.a.e[ix],&v->u.a.e[ix+count]);
    }
    while(count>0)
    {
        --count;
        lept_free(&v->u.a.e[ix+count]);
        --v->u.a.size; 
    }
}
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    /* \todo */
    lept_value* res = lept_find_object_value(v, key, klen);
    ...
    {
        for(;ix < v->u.o.size;++ix)
        {
            if(v->u.o.m[ix].k == NULL)break;
        }
    }
    v->u.o.m[ix].k = (char*)malloc(sizeof(char)*klen);
    memcpy(v->u.o.m[ix].k, key, klen);
    v->u.o.m[ix].klen = klen;
    ++v->u.o.size;
    return &v->u.o.m[ix].v;
}
...
void lept_remove_object_value(lept_value* v, size_t index) {
    ...
    size_t ix = index;
    for(;ix+1 < v->u.o.size;++ix)
    {
        v->u.o.m[ix] = v->u.o.m[ix+1];
    }
    --v->u.o.size;
    return;
}
